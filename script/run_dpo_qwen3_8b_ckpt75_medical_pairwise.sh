...
TRAIN_DATA="${TRAIN_DATA:-${PROJECT_ROOT}/data/alignment/processed/dpo/train/medical_pairwise_train.processed.jsonl}"
VALID_DATA="${VALID_DATA:-${PROJECT_ROOT}/data/alignment/processed/dpo/valid/medical_pairwise_valid.processed.jsonl}"
...
RUN_NAME="${RUN_NAME:-$(date +%Y%m%d_%H%M%S)_qwen3-8b_ckpt75_medical_pairwise_dpo}"
...
"${TORCHRUN_BIN}" \
  ...
  "${PROJECT_ROOT}/script/train_dpo.py" \
  "${TRAIN_ARGS[@]}" \
